...
class Login extends CI_Controller
{
  ...
  {
    $session = $this->session->userdata('isLoginkeuanganSD');
      ...
      {
        redirect('sd/keuangan/dashboard');
      }
  }
  ...
  public function do_login()
  {
    ...
    $this->form_validation->set_error_delimiters('<span class="error">', '</span>');
      ...
      {
        $this->load->view('sd/keuangan/view_login');
      }else
      {
        ...
        if($cek > 0){
          if($cek->LEVEL == 'KEUSD'){
            if($cek->USERNAME == $username){
              ...
              $this->session->set_userdata('isLoginKeuanganSD', TRUE);
