...
class Login extends CI_Controller
{
  ...
  {
    $session = $this->session->userdata('isLoginAdminPaud');
  ...
  public function do_login()
  {
    ...
    $this->form_validation->set_error_delimiters('<span class="error">', '</span>');
      ...
      {
        $this->load->view('paud/admin/view_login');
      }else
      {
       ...
       //print_r($cek);die;
       if($cek > 0){
            if($cek->LEVEL == 'ADMINPAUD'){
                if($cek->USERNAME == $username){
                      if($cek->PASSWORD == $password){
                              $this->session->set_userdata('isLoginAdminPaud', TRUE);
