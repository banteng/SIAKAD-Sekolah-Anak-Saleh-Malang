...
class Chat extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!isset($this->session->userdata['isLoginAdmin']) || $this->session->userdata['isLoginAdmin'] === false) {
            redirect('welcome');
        }

        $this->user = $this->db->get_where('a_admin', array('ID' => $this->session->userdata['USERNAME']), 1)->row();
    }
