...
class Model_all extends CI_Model
{
	...
	{
		return $this->db->get_where('tb_tahun_aktif', array('STATUS' => 1, 'LEVEL' => "PAUD"))->row();
	}
...
//START MODEL SISWA
	...
	{
		$query = $this->db->get_where('view_siswa1', array('LEVEL' => "PAUD"));
		return $query->result();
	}
    
    public function getSiswaByKelas($kelas)
	{
		$query = $this->db->get_where('view_siswa1', array('ID_KELAS' => $kelas,'LEVEL' => "PAUD"));
		return $query->result();
	}

	public function getAllKelas()
	{
		$query = $this->db->query('SELECT * FROM tb_kelas WHERE LEVEL="PAUD" ');
		return $query->result();
	}
    ...
    public function getAllKelasPAUD()
	{
		$query = $this->db->query('SELECT * FROM tb_kelas WHERE LEVEL="PAUD" ');
		return $query->result();
	}
...
public function getAllPegawai()
	{
		$query = $this->db->get_where('view_pegawai1', array('LEVEL' => "PAUD"));
		return $query->result();
	}
...
// START MODEL KELAS
	...
	{
		$query = $this->db->get_where('view_kelas1', array('LEVEL' => "PAUD"));
		return $query->result();
	}
...
{
	return $this->db->get_where('pengumuman', array('ID' => $id))->row();
}
...
{
    return $this->db->query("SELECT * FROM `view_pembayaran` WHERE LEVEL = 'PAUD' ORDER BY TANGGAL ASC LIMIT 10")->result();
}
...
{
	return $this->db->get_where('view_enroll_spp', array('NIS' => $id))->row();
}
...
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "PAUD", "TAHUNAKTIF" => $tahun));
	return $query->result();
}

public function LaporanPembayaranbyId($nis, $tahun)
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "PAUD", "NIS"=> $nis, "TAHUNAKTIF" => $tahun));
	return $query->result();
}
public function cekPembayaranbyId($nis, $bulan)
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "PAUD", "BULAN"=> $nis, "BULAN"=> $bulan));
	return $query->result();
}
function getTagihanSppPaud()
{
	/*$this->db->select('
	...
	$query = $this->db->get();
	return $query->result();*/
	return $this->db->get_where('view_enroll_spp_2', array('LEVEL' => "PAUD"))->result();
}
...
function getPembayaranKB($tahun)
{
	//$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "PAUD", 'TAHUNAKTIF' => $tahun));
    $query = $this->db->query("SELECT * FROM view_pembayaran WHERE LEVEL='PAUD' AND TAHUNAKTIF='".$tahun."' GROUP BY NIS");
	return $query->result();
}
    
function getPembayaranStatus($tahun)
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "PAUD", 'TAHUNAKTIF' => $tahun));
    //$query = $this->db->query("SELECT * FROM view_pembayaran WHERE LEVEL='PAUD' AND TAHUNAKTIF='".$tahun."' GROUP BY NIS");
	return $query->result();
...
public function getEnrollSPP()
	{
		$query = $this->db->get_where('view_enroll_spp_2', array('LEVEL' => "PAUD"));
		return $query->result();
	}
...
public function getAllUser()
    ...
    {
        $query = $this->db->query('SELECT * FROM view_siswa1 WHERE LEVEL= "PAUD" ');
    ...
    function countAllSPP()
    {
         ...
         ->where('LEVEL', "PAUD");
         $query = $this->db->get('view_pembayaran');
         return $query->result();*/
         $query = $this->db->query('SELECT SUM(NOMINAL) as NOMINAL FROM view_pembayaran WHERE LEVEL= "PAUD" ')->row();
         //return $query->result_array();
    }
    
     function countAllKelas()
    {
        $query = $this->db->query('SELECT * FROM view_kelas1 WHERE LEVEL= "PAUD" ');
        return $query->num_rows();
    }
    
     function countAllPegawai()
    {
        $query = $this->db->query('SELECT * FROM tb_pegawai WHERE LEVEL = "PAUD" ');
        return $query->num_rows();
    }
    ...
    {
        $query = $this->db->query('SELECT * FROM view_siswa1 WHERE ID_KELAS=" " AND LEVEL="PAUD" ');
        return $query->num_rows();
    }
    ...
    function tambahPesan($table, $data)
 	{   
		$this->db->insert($table, $data);
	}
