...
class Model_all extends CI_Model
{
	...
	{
		return $this->db->get_where('tb_tahun_aktif', array('STATUS' => 1,'LEVEL' => "SD"))->row();
	}
...
//START MODEL SISWA
	...
	{
		$query = $this->db->get_where('view_siswa1', array('LEVEL' => "SD"));
		return $query->result();
	}
    
      public function getSiswaByKelas($kelas)
	{
		$query = $this->db->get_where('view_siswa1', array('ID_KELAS' => $kelas, 'LEVEL' => "SD"));
		return $query->result();
	}
    ...
    public function getAllKelasSD()
	{
		$query = $this->db->get_where('tb_kelas', array('LEVEL' => "SD"));
		return $query->result();
	}
...
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "SD", "TAHUNAKTIF" => $tahun));
	return $query->result();
}
...
function getTagihanSPP()
{
	...
	$query = $this->db->get();
	return $query->result();*/
	return $this->db->get('view_enroll_spp')->result();
}
...
function getPembayaranSD($tahun)
{
	$query = $this->db->get_where('view_pembayaran', array('LEVEL' => "SD", 'TAHUNAKTIF' => $tahun));
	return $query->result();
}
...
public function getEnrollSPP()
	{
		$query = $this->db->get_where('view_enroll_spp_2', array('LEVEL' => "SD"));
		return $query->result();
	}
...
public function getAllUser()
    ...
    {
        $query = $this->db->query('SELECT * FROM view_siswa1 WHERE LEVEL= "SD" ');
        return $query->num_rows();
    }
    
     function countAllKelas()
    {
        $query = $this->db->query('SELECT * FROM view_kelas1 WHERE LEVEL= "SD"' );
        return $query->num_rows();
    }
    
     function countAllPegawai()
    {
        $query = $this->db->query('SELECT * FROM tb_pegawai WHERE LEVEL= "SD" ');
        return $query->num_rows();
    }
    ...
    {
        $query = $this->db->query('SELECT * FROM view_siswa1 WHERE ID_KELAS=" " AND LEVEL="SD" ');
        return $query->num_rows();
    }
    ...
    {
        return $this->db->query("SELECT * FROM `view_pembayaran` WHERE LEVEL = 'SD' ORDER BY TANGGAL ASC LIMIT 10")->result();
    }
    ...
    function getTagihanSppTk()
    {
        ...
        $query = $this->db->get();
        return $query->result();*/
        return $this->db->get_where('view_enroll_spp_2', array('LEVEL' => "SD"))->result();
    }                               

    function getTagihanSppSd()
    {
        /*$this->db->select('
        tb_enroll_spp.*, tb_siswa.NIS AS NIS, tb_siswa.NAMA_SISWA
        ');
