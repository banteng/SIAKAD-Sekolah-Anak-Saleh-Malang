...
?>
				...
				<div class="box box-primary">
					...
					if (!empty($data)) :
						...
						foreach ($data as $d):
							...
							echo '<input type="hidden" name="section_'.$d->section.'" value="'.$d->section.'">';
					?>
					...
					<div class="box-body box-ujian step">
						...
						<div><?=$d->soal;?></div>
                           <?php if ($exmedia == 'mp3') {
                            ...
                            if ($exmedia == 'mp4') {
								echo '<audio src="'.base_url('media/'.$d->media).'" class="" controls="controls" id="myaudio"></audio>';
							}
                        ?>
						<?php
		...
		<script>
			...
			simpan = function(id){
				...
				$.ajax({
					type: 'POST',
					url: base_url+'user/ujian_simpan_1/'+id,
			...
			simpan_akhir = function(id){
				if (confirm('Are you sure want to end this question ?')) {
					...
					$.ajax({
						type: 'POST',
						url: base_url+'user/ujian_akhir_1/'+id,
						data: JSON.stringify(form),
						dataType: 'json',
						contentType: 'application/json; charset=utf-8'
					}).done(function(response){
