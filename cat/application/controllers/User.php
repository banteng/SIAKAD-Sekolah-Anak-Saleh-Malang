...
class User extends CI_Controller {
	...
	function notfound(){
     ...
     public function index(){
     	$id = array('id_siswa' => $this->session->id);
     	$data['ar'] = $this->m_user->cekdefpass($id)->row_array();
     	$data['title'] = 'International Student Service';
          $data['judul'] = 'Dashboard';
          $whrkelas = array('kelas.kode_kelas' => $this->session->userdata('kelas'));
     ...
     public function ujian($id_ujian){
          $c = $this->m_user->cek_detil_tes(array('id_ujian' => $id_ujian));
          if ($c->num_rows() < 1) {
               # code...
               redirect('');
          }
          ...
          $whr_cek_sdh_selesai = array('id_siswa' => $data['sess_id'], 'id_ujian' => $id_ujian, 'status' => 'N');
          ...
          if ($cek_sdh_selesai < 1) {
               ...
               $whr_cek_detil_tes = array('id_ujian' => $id_ujian);
               $cek_detil_tes = $this->m_user->cek_detil_tes($whr_cek_detil_tes)->row();

               $whr_cek_sdh_ujian = array('id_ujian' => $id_ujian, 'id_siswa' => $data['sess_id']);
               ...
               if ($cek_sdh_ujian->num_rows() < 1) {
                    ...
                    $q_soal = $this->m_user->q_soal($cek_detil_tes->id_mapel, $kelas->kelas, "1")->result();
                    $i = 0;
                    foreach ($q_soal as $q) {
                         # code...
                    ...
                    $data_tmbh_ujian = array(
                         'id_ujian' => $id_ujian,
                         'id_siswa' => $data['sess_id'],
                         'list_soal_1' => $list_id_soal,
                         'list_jawaban_1' => $list_jw_soal,
                         'benar_section_1' => 0,
                         'benar_section_2' => 0,
                         'benar_section_3' => 0,
                         'jml_benar' => 0,
                    ...
                    $urut_soal = explode(",", $q_ambil_soal->list_jawaban_1);
                    $soal_urut_ok = array();
                    ...
                    $whr_detil_soal = array('id_ujian' => $id_ujian);
               ...
               $xx = $this->m_user->cek_selesai_ujian(array('id_siswa' => $data['sess_id'], 'id_ujian' => $id_ujian, 'status' => 'N'))->row();
     ...
     public function ujian_selesai($id_tes){
          if ($data['detil_tes'] = $this->m_user->detil_tes(array('id_tes' => $id_tes))->num_rows() < 1) {
               redirect('');
          }
          $data['detil_tes'] = $this->m_user->detil_tes(array('id_tes' => $id_tes))->row();
         //print_r($data['detil_tes'] );die;
          $whr_detil_soal = array('id_ujian' => $data['detil_tes']->id_ujian);
          $data['detil_soal'] = $this->m_user->detil_soal($whr_detil_soal)->row();
     ...
     function gantipass(){
     	...
     	$whr = array('id_siswa' => $this->session->id);;
